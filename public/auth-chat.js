...
socket.on("connect", () => {
    ...
    socket.emit(
        "checkin",
        { friendship_id, token: getCookie("token") },
        (err, data) =>
            !err
...
socket.on("newMessage", data => {
    ...
    socket.emit(
        "gotMessage",
        { friendship_id, token: getToken(), Ids: data.Ids },
...
$("#friend-form").submit(e => {
    ...
    $.ajax({
        ...
        headers: {
            ...
            /** @todo why am I sending a cookie in the headers? */
            "x-auth": getToken()
        },
...
$("#message-form").submit(e => {
    ...
    if (text.trim().length > 0) {
        ...
        if (hID) {
            ...
            var quotedMessage = messageNode.querySelector(
                ".message__body .wrap"
            ).innerHTML;
            templateData.quotedFrom = nameAt[0].innerHTML;
            templateData.quotedAt = new Date(
                parseInt(nameAt[1].innerHTML)
            ).toLocaleTimeString();
...
$("#my-emojis").click(e => {
    if (e.target.dataset.value) {
        $("#msg-txt")
            .val($("#msg-txt").val() + e.target.dataset.value)
            .focus();
    }
});
